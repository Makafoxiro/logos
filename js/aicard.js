...
function buildAICard(mode) {
  ...
  const skillTpl     = g('sportTemplates', null) || g('skillTemplates', []);
      ...
      if (physTpl.length) {
        ...
        physTpl.forEach(t => {
          ...
          if ((!t.exercisesHeavy || !t.exercisesHeavy.length) && (!t.exercisesLight || !t.exercisesLight.length)) {
            // Берём первый непустой из: exercises, exercisesNone
            const exArr = (Array.isArray(t.exercises) && t.exercises.length) ? t.exercises
                        : (Array.isArray(t.exercisesNone) && t.exercisesNone.length) ? t.exercisesNone : [];
            exArr.forEach(ex => lines.push(`    - ${ex}`));
          }
