...
function renderTodayTasks(now){
  ...
  // ── СЕГОДНЯ ──
  html += sectionHeader('сегодня', 'today', _taskSections.today, realTodayTasks.filter(t=>!t.done).length, false);
  if(_taskSections.today){
    ...
    if(!realTodayTasks.length){
      html += `<div style="${INNER_EMPTY}">нет задач — добавь во вкладке «неделя»</div>`;
    } else {
  ...
  // ── ЗАПЛАНИРОВАНО ──
  html += sectionHeader('запланировано', 'planned', _taskSections.planned, plannedDays.reduce((s,{key})=>s+((typeof tasks!=='undefined'&&tasks[key])?tasks[key].length:0),0), false);
  if(_taskSections.planned){
    html += `<div style="${INNER};display:flex;flex-direction:column;gap:6px;padding-top:8px;padding-bottom:8px">`;
    if(!plannedDays.length){
      html += `<div style="${INNER_EMPTY}">нет запланированных задач</div>`;
    } else {
  ...
  html += sectionHeader('невыполненные', 'undone', _taskSections.undone, undoneCount, false);
  if(_taskSections.undone){
    html += `<div style="${INNER};display:flex;flex-direction:column;gap:6px;padding-top:8px;padding-bottom:8px">`;
    if(!undoneDays.length){
      html += `<div style="${INNER_EMPTY}">всё выполнено 👍</div>`;
    } else {
      undoneDays.forEach(({key, date}) => {
        ...
        html += dayTasks.map(t => {
          const carryBtn = `<button onclick="event.stopPropagation();carryTaskToToday('${key}','${t.id}')" title="Перенести на сегодня" style="flex-shrink:0;width:32px;height:32px;background:var(--c2);border:1px solid var(--bd);border-radius:8px;color:var(--a);font-size:13px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:.15s" onmouseenter="this.style.background='var(--a)';this.style.color='#000'" onmouseleave="this.style.background='var(--c2)';this.style.color='var(--a)'">↑</button>`;
          return `<div class="today-item" id="today-undone-${t.id}" onclick="toggleTodayTask('${key}','${t.id}')">
            <div class="chk"></div>
            <div style="font-size:15px">${t.emoji||''}</div>
            <div class="item-name" style="flex:1;min-width:0">${t.name}</div>
            ${carryBtn}
          </div>`;
        }).join('');
      });
...
// ── ПЕРЕНОС ЗАДАЧИ ИЗ НЕВЫПОЛНЕННЫХ В СЕГОДНЯ ────────────
function carryTaskToToday(oldKey, taskId){
  if(typeof tasks==='undefined') return;
  const arr = tasks[oldKey] || [];
  const idx = arr.findIndex(x => String(x.id) === String(taskId));
  if(idx === -1) return;
  const t = arr[idx];

  // Удаляем из старого дня полностью
  tasks[oldKey].splice(idx, 1);
  if(!tasks[oldKey].length) delete tasks[oldKey];

  // Добавляем в сегодня если ещё нет
  const todayKey = dk2(new Date());
  if(!tasks[todayKey]) tasks[todayKey] = [];
  const alreadyThere = tasks[todayKey].some(x => x.name === t.name);
  if(!alreadyThere){
    ...
    tasks[todayKey].push({...t, done: false, id: newId, _carriedOver: false});
  }
  if(typeof LS !== 'undefined') LS.s('tasks', tasks);

  // Анимация исчезновения, потом перерисовка
  const el = document.getElementById('today-undone-' + taskId);
  if(el){
    todayFadeOut(el, () => renderTodayTasks(new Date()));
  } else {
    renderTodayTasks(new Date());
  }
}
