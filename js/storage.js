// ═══ MODULE: storage.js ═══
const LS={g(k,d){try{const v=localStorage.getItem(k);return v?JSON.parse(v):d}catch{return d}},s(k,v){localStorage.setItem(k,JSON.stringify(v))}};
...
(function _migrateSkillExercises(){
  ...
  skillTemplates.forEach(t=>{
    if(!t.exercises||!t.exercises.length){
      ...
      const src=wi==='heavy'?t.exercisesHeavy:wi==='light'?t.exercisesLight:t.exercisesNone;
      if(src&&src.length){t.exercises=src;changed=true;}
    }
  });
