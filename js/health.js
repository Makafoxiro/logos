...
function renderSleepCups(sleptHrs, normHrs){
  ...
  if(badge){
    if(hrs>0){
      ...
      // Сон пересекает полночь — всегда показываем за какую ночь данные
      const _now=new Date();
      const _prev=new Date(_now); _prev.setDate(_prev.getDate()-1);
      const _dd=d=>String(d.getDate()).padStart(2,'0')+'.'+String(d.getMonth()+1).padStart(2,'0');
      const nightLabel = _now.getHours()<12
        ? 'ночь '+_dd(_prev)+'–'+_dd(_now)
        : 'ночь '+_dd(_prev)+'–'+_dd(_now);  // всегда за прошлую ночь
      badge.innerHTML='<span style="color:'+col+';background:'+bg+';border:1px solid '+bd+';font-family:var(--mono);font-size:10px;padding:2px 8px;border-radius:10px">'+(ok?'✓ норма':'↓ мало')+'</span>'
        +'<span style="color:var(--t3);font-family:var(--mono);font-size:10px;padding:2px 8px;border-radius:10px;border:1px solid var(--bd);background:var(--c2);margin-left:6px">'+nightLabel+'</span>';
    } else { badge.innerHTML=''; }
  }
}

function calcKBZHU(weight, trainDays){
  // Миффлин-Сен Жеор — наиболее точная формула базального метаболизма
  ...
  // ── TDEE ─────────────────────────────────────────────────────────────────
  // Коэффициенты активности Harris-Benedict (валидированные)
  ...
  // ── БЕЛОК: зависит от тренировочной нагрузки, возраста, режима и пола ────
  // Источники: ISSN (2017), Stokes et al., WHO/FAO
  //   0 тр/нед  → 1.2 г/кг (малоподвижный образ жизни, норма ВОЗ)
  //   1-2 тр    → 1.6 г/кг (лёгкая активность)
  //   3-4 тр    → 2.0 г/кг (умеренные силовые)
  //   5-7 тр    → 2.3 г/кг (высокая нагрузка, ISSN верхняя граница)
  let protBase = td === 0 ? 1.2 : td <= 2 ? 1.6 : td <= 4 ? 2.0 : 2.3;
  // Возраст: подростки (<18) — синтез белка эффективнее, норма чуть ниже
  //          пожилые (≥65) — анаболическая резистентность, нужно больше
  if (age < 18)       protBase = Math.max(1.2, protBase - 0.2);
  else if (age >= 65) protBase = Math.min(2.5, protBase + 0.2);
  // Пол: женщинам немного меньше (меньше мышечная масса относительно веса)
  if (gender === 'female') protBase = Math.max(1.2, protBase - 0.2);
  // Режим: сушка → больше белка для сохранения мышц; набор → чуть меньше
  if (mode === 'cut')  protBase = Math.min(2.5, protBase + 0.3);
  if (mode === 'bulk') protBase = Math.max(1.2, protBase - 0.1);
  const protPerKg = Math.round(protBase * 10) / 10;
  const prot = Math.round(weight * protPerKg);

  // ── ЖИРЫ: зависят от режима и пола ──────────────────────────────────────
  // Норма: 0.8–1.2 г/кг. Сушка → минимум; набор → чуть больше для гормонов
  let fatBase = gender === 'female' ? 1.1 : 1.0;
  if (mode === 'cut')  fatBase = Math.max(0.7, fatBase - 0.2);
  if (mode === 'bulk') fatBase = Math.min(1.3, fatBase + 0.1);
  const fatPerKg = Math.round(fatBase * 10) / 10;
  ...
  return {kcal, prot, fat, carb, water, coeff: coeff.toFixed(2), tdee, deficit, surplus, mode, bmr: Math.round(bmr), protPerKg, fatPerKg};
}

// Расчёт норм микронутриентов — Academy of Nutrition, ВОЗ, AHA
function calcMicros(weight, age, kcal){
  const targetKcal = kcal || health.kcal || 2500;
  const trainDays  = health.trainDays || 4;

  // КЛЕТЧАТКА: 14 г / 1000 ккал (Academy of Nutrition and Dietetics, 2015)
  // Диапазон 25-45 г/день — нижняя граница ВОЗ, верхняя без вздутия
  const fiber = Math.round(Math.min(45, Math.max(25, targetKcal / 1000 * 14)));

  // САХАР: ВОЗ <10% суточных ккал из добавленных сахаров (1 г = 4 ккал)
  // Пол и возраст (AHA): мужчины ≤36 г, женщины ≤25 г, подростки ≤24 г
  const ageVal    = (typeof age === 'number' && age > 0) ? age : (health.age || 17);
  const gnd       = health.gender || 'male';
  const ahaLimit  = ageVal < 18 ? 24 : (gnd === 'female' ? 25 : 36);
  const whoLimit  = Math.round(targetKcal * 0.10 / 4);
  const sugar     = Math.min(ahaLimit, Math.max(20, whoLimit));

  // СОЛЬ: ВОЗ <5 г/день. Спортсменам +0.5 г при ≥4 тр/нед (потери с потом)
