...
function goPage(name,btn){
  ...
  // Activate real button + all clones of this page
  const realBtn=document.getElementById('nb-'+name);
  if(realBtn)realBtn.classList.add('on');
  document.querySelectorAll('.nb[data-nav-page="'+name+'"]').forEach(c=>c.classList.add('on'));
  curPage=name;
  if(name==='memo'&&pinState==='enter'){memoUnlocked=false;curMemoTab='birthdays';}
  renderCurrent();
  // Scroll real nav button into center view
  const nav=document.querySelector('.nav');
  if(realBtn&&nav){
    const btnLeft=realBtn.offsetLeft;
    const btnW=realBtn.offsetWidth;
    const navW=nav.offsetWidth;
    const target=btnLeft-navW/2+btnW/2;
    nav.scrollTo({left:Math.max(0,target),behavior:'smooth'});
  }
}

// ══ INFINITE NAV SCROLL ═══════════════════════════════
// Only on touch devices where nav actually overflows.
// Clone buttons before+after so scrolling appears endless.
// When user drifts into clone zone, silently teleport back to real set.
(function initInfiniteNav(){
  const nav=document.querySelector('.nav');
  if(!nav)return;
  // Only activate on touch devices
  if(!('ontouchstart' in window)&&!navigator.maxTouchPoints)return;

  const origBtns=Array.from(nav.querySelectorAll('.nb'));
  if(!origBtns.length)return;

  function getPage(btn){
    const m=(btn.getAttribute('onclick')||'').match(/goPage\('(\w+)'/);
    return m?m[1]:'';
  }

  function makeClone(btn){
    const clone=btn.cloneNode(true);
    clone.removeAttribute('id');
    const page=getPage(btn);
    clone.dataset.navPage=page;
    clone.setAttribute('onclick',"goPage('"+page+"',document.getElementById('nb-"+page+"'))");
    return clone;
  }

  // Prepend one set + append one set
  const fragment=document.createDocumentFragment();
  origBtns.forEach(b=>fragment.appendChild(makeClone(b)));
  nav.appendChild(fragment);

  const fragment2=document.createDocumentFragment();
  origBtns.forEach(b=>fragment2.appendChild(makeClone(b)));
  nav.insertBefore(fragment2,nav.firstChild);

  // Measure one set width and jump to real buttons
  let setWidth=0;
  let jumping=false;

  function measureAndInit(){
    const cloneBefore=Array.from(nav.children).slice(0,origBtns.length);
    setWidth=cloneBefore.reduce((s,el)=>s+el.offsetWidth,0);
    if(setWidth===0){requestAnimationFrame(measureAndInit);return;}
    nav.scrollLeft=setWidth;
  }
  requestAnimationFrame(()=>requestAnimationFrame(measureAndInit));

  nav.addEventListener('scroll',()=>{
    if(jumping||setWidth===0)return;
    const sl=nav.scrollLeft;
    if(sl<setWidth*0.4){
      jumping=true;
      nav.scrollLeft=sl+setWidth;
      requestAnimationFrame(()=>{jumping=false;});
    } else if(sl>setWidth*1.6){
      jumping=true;
      nav.scrollLeft=sl-setWidth;
      requestAnimationFrame(()=>{jumping=false;});
    }
...
// ══ NAV SCROLL FIX: tap vs scroll on mobile ═══════════
(function fixNavTouchScroll(){
  const nav=document.querySelector('.nav');
  if(!nav)return;
  ...
  nav.addEventListener('touchmove',e=>{
    ...
    if(dx>8&&dx>dy)didScroll=true;
  },{passive:true});
  nav.addEventListener('click',e=>{
    if(didScroll){e.stopPropagation();e.preventDefault();didScroll=false;}
  },true);
...
(function initSwipe(){
  ...
  document.addEventListener('touchend',e=>{
    ...
    // Don't trigger page-swipe if touch started inside the nav bar (user is scrolling nav)
    if(touchStartEl&&touchStartEl.closest('.nav'))return;
    ...
    if(Math.abs(dx)>60&&Math.abs(dx)>Math.abs(dy)*1.5&&dt<450){
      ...
      const next=dx>0?(idx+1)%pages.length:(idx-1+pages.length)%pages.length;
      if(next!==idx){
...
function collectNotifications(){
  ...
  (function(){
    ...
    // Новый месяц — по аналогии с неделей
    let monthName;
    if(daysLeft<=0){monthName='Сегодня последний день месяца!';}
    else if(daysLeft===1){monthName='Завтра новый месяц!';}
    else if(daysLeft<=3){monthName='Скоро новый месяц — '+daysLeft+'д.';}
    else{monthName='Скоро новый месяц';}
    notifs.push({
      id:'month_newmonth', name:monthName,
...
// ══ PRE-INIT VARS (must be defined before renderCurrent/goPage to avoid TDZ) ══
let appTheme = LS.g('appTheme','dark');
let appLang  = LS.g('appLang','ru');
...
// Wrap all init logic in try/finally so splash always hides
(function _init(){
...
const T = {
  ru:{
    ...
    today_fg_lbl:'просто цели',today_dl_lbl:'цели с дедлайном',
    ...
    health_kcal:'Калории',health_kcal_unit:'ккал',health_prot:'Белок',health_fat:'Жиры',health_carb:'Углеводы',health_water_glasses:'стаканов',
  ...
  en:{
    logo:'LOGOS',
    nav:['TODAY','ROUTINE','WEEK','GOALS','HABITS','HEALTH','SKILLS','INTERESTS','MONEY','NOTES','SETTINGS'],
    ...
    today_fg_lbl:'free goals',today_dl_lbl:'deadline goals',
    ...
    health_kcal:'Calories',health_kcal_unit:'kcal',health_prot:'Protein',health_fat:'Fats',health_carb:'Carbs',health_water_glasses:'glasses',
...
function applyLang(){
  ...
  // Nav buttons text — skip clones (they have data-nav-page attr)
  document.querySelectorAll('.nb:not([data-nav-page])').forEach((btn,i)=>{
    if(lang.nav[i]){
  ...
  // Sync translated text into clones (they mirror real buttons)
  document.querySelectorAll('.nb:not([data-nav-page])').forEach((realBtn,i)=>{
    const page=realBtn.id.replace('nb-','');
    if(!page)return;
    let txt='';
    for(let j=realBtn.childNodes.length-1;j>=0;j--){
      if(realBtn.childNodes[j].nodeType===3){txt=realBtn.childNodes[j].textContent;break;}
    }
    if(!txt)return;
    document.querySelectorAll('.nb[data-nav-page="'+page+'"]').forEach(clone=>{
      for(let j=clone.childNodes.length-1;j>=0;j--){
        if(clone.childNodes[j].nodeType===3){clone.childNodes[j].textContent=txt;return;}
      }
    });
  });
  ...
  _ph('freeGoalTextInp','today_no_tasks');
