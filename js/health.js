...
function renderSleepCups(sleptHrs, normHrs){
  ...
  // Подсказка: сон с 22:00 до 09:00 пересекает полночь — показываем за какой день
  const now = new Date();
  const isEarlyMorning = now.getHours() < 12;
  const sleepDayLabel = isEarlyMorning
    ? (() => { const d=new Date(now); d.setDate(d.getDate()-1); return 'ночь '+d.getDate()+'.'+(d.getMonth()+1<10?'0':'')+(d.getMonth()+1); })()
    : null;
  if(subEl) subEl.textContent='из '+norm+' часов'+(sleepDayLabel?' · '+sleepDayLabel:'');
