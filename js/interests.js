...
function renderInterests() {
  ...
  if (!d.cats.length) {
    bar.innerHTML = '';
    content.innerHTML = '<div class="int-empty">'+(typeof t==='function'?t('interests_empty_cats'):'нет категорий — создай первую')+'</div>';
    intActiveCat = null;
    return;
  }
  ...
  content.innerHTML = `
    <div class="int-cat-header">
      ...
      ${totalItems > 0 ? `<div class="int-cat-count">${totalItems} ${totalItems===1?(typeof t==='function'?t('interests_item'):'пункт'):totalItems<5?(typeof t==='function'?t('interests_items2'):'пункта'):(typeof t==='function'?t('interests_items5'):'пунктов')}</div>` : ''}
    </div>
    ${items.length === 0
      ? '<div class="int-empty">'+(typeof t==='function'?t('interests_empty_items'):'нет пунктов — добавь первый')+'</div>'
      : `<div class="int-grid">${items.map((item, idx) => `
          <div class="int-card" onclick="openEditIntItem('${item.id}')">
            <div class="int-card-num">${idx + 1}</div>
...
function openAddInterestCat() {
  document.getElementById('intCatModalTitle').textContent = (typeof t==='function'&&appLang==='en'?'new category':'новая категория');
...
function openEditIntCat(id) {
  ...
  if (!cat) return;
  document.getElementById('intCatModalTitle').textContent = (typeof t==='function'&&appLang==='en'?'edit category':'редактировать категорию');
...
function openAddIntItem(catId) {
  document.getElementById('intItemModalTitle').textContent = (typeof t==='function'&&appLang==='en'?'new item':'новый пункт');
...
function openEditIntItem(id) {
  ...
  if (!item) return;
  document.getElementById('intItemModalTitle').textContent = (typeof t==='function'&&appLang==='en'?'edit item':'редактировать пункт');
