  ...
  pastKeys.forEach(k => {
    ...
    unfinished.forEach(t => {
      if(!todayNames.includes(t.name)){
        ...
        // Копия НЕ получает _carriedOver, чтобы завтра снова перенеслась если не выполнена
...
function carryTaskToToday(oldKey, taskId){
  ...
  // Помечаем в старом дне как перенесённую
  t._carriedOver = true;
  if(typeof LS !== 'undefined') LS.s('tasks', tasks);
  ...
  if(el) {
    todayFadeOut(el, () => {
      ...
      const todayKey = dk2(new Date());
      if(!tasks[todayKey]) tasks[todayKey] = [];
      const alreadyThere = tasks[todayKey].some(x => String(x.id) === String(taskId) || x.name === t.name);
      if(!alreadyThere){
        const newId = (typeof nid !== 'undefined') ? nid++ : (Date.now() + Math.floor(Math.random()*1000));
        if(typeof nid !== 'undefined' && typeof LS !== 'undefined') LS.s('nid', nid);
        tasks[todayKey].push({...t, done: false, id: newId, _carriedOver: false});
