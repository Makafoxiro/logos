...
function buildAICard() {
  ...
  if (hasHealth) {
    ...
    if (sleepSchedule.bed || sleepSchedule.wake) {
      ...
      kv('Расписание сна', `отбой ${sleepSchedule.bed || '—'} / подъём ${sleepSchedule.wake || '—'}${napStr}`);
    }
    ...
    if (hLogDays.length) {
      h2('Факт питания и сна (последние дни)');
  ...
  if (activeGoals.length || pendingFree.length || annuals?.length) {
    ...
    if (Array.isArray(annuals) && annuals.length) {
      h2('Ежегодные напоминания');
      const _pLabel = p => { if(!p)return ''; if(p==='3m')return 'каждые 3 мес.'; if(p==='6m')return 'каждые 6 мес.'; const y=parseInt(p); return y?'каждые '+y+' лет':''; };
      ...
      annuals.forEach(a => {
        ...
        if (a.period && a.lastDate) {
          ...
          if (!isNaN(_d)) {
            ...
            const _sign=_days<0?'просрочено '+Math.abs(_days)+' дн.':'через '+_days+' дн.';
            meta=' | след. '+_d.getDate()+'.'+(_d.getMonth()+1)+'.'+_d.getFullYear()+' ('+_sign+') | '+_pLabel(_p);
          }
        } else if (a.date) {
          const _d2=new Date(a.date);
          if(!isNaN(_d2)){const _days2=Math.round((_d2-_nowA)/86400000);const _sign2=_days2<0?'просрочено':'через '+_days2+' дн.';meta=' | '+_d2.getDate()+'.'+(_d2.getMonth()+1)+'.'+_d2.getFullYear()+' ('+_sign2+')';}
        }
        li(a.name + meta);
      });
    }

    // Цели текущего месяца
    const nowMK = `M${new Date().getFullYear()}-${new Date().getMonth()}`;
    const monthGoals = (mg[nowMK] || []);
    if (monthGoals.length) {
      h2('Цели этого месяца');
  ...
  if (hasFinance) {
    ...
    // Бюджетный лимит
    if (finBudget > 0) kv('Месячный бюджет (лимит)', finBudget.toLocaleString('ru') + ' руб.');

    // Категории бюджета (круговая диаграмма)
    const activePie = finPieSlices.filter(s => s.name && s.pct > 0);
    if (activePie.length) {
      h2('Категории бюджета');
      activePie.forEach(s => {
        const desc = s.desc ? ` — ${s.desc}` : '';
    ...
    activeHW.forEach(h => {
      ...
      if (h.date) {
        ...
        if (!isNaN(d)) {
          ...
          dl = days < 0 ? ` [просрочено ${Math.abs(days)} дн.]` : days === 0 ? ' [сегодня сдать!]' : ` [через ${days} дн.]`;
        }
      }
      li(`${h.subj}: ${h.task}${dl}`);
    });
    ...
    activePatches.forEach(p => {
      ...
      if (p.date) {
        ...
        if (!isNaN(d)) {
          ...
          dl = days < 0 ? ` [просрочено]` : days === 0 ? ' [сегодня]' : ` [через ${days} дн.]`;
        }
      }
  ...
  if (birthdays.length) {
    ...
    sorted.forEach(b => {
      ...
      const gifts = (b.gifts || []);
      const giftStr = gifts.length ? ` | 🎁 идеи: ${gifts.join(', ')}` : '';
  ...
  // Цели: похожие / дублирующие записи (Jaccard по словам, порог 50%)
  {
    // Нормализация: строчные, убираем спецсимволы, разбиваем на слова ≥3 букв
    const normalize = str => str.toLowerCase().replace(/[^а-яёa-z0-9\s]/g, ' ')
      .split(/\s+/).filter(w => w.length >= 3);

    // Jaccard similarity между двумя наборами слов
    const jaccard = (a, b) => {
    ...
    // Все активные цели (обычные + свободные) объединяем
    const allGoalItems = [
      ...activeGoals.map(g => ({ src: 'цель', name: g.name })),
      ...pendingFree.map(g  => ({ src: 'своб.цель', name: g.name })),
    ];

    const similarPairs = [];
    for (let i = 0; i < allGoalItems.length; i++) {
      for (let j = i + 1; j < allGoalItems.length; j++) {
        const wa = normalize(allGoalItems[i].name);
        const wb = normalize(allGoalItems[j].name);
        // Точный дубликат или высокое пересечение слов
        const sim = jaccard(wa, wb);
        if (sim >= 0.5 || allGoalItems[i].name.toLowerCase().trim() === allGoalItems[j].name.toLowerCase().trim()) {
          similarPairs.push(`«${allGoalItems[i].name}» ≈ «${allGoalItems[j].name}»`);
        }
      }
    }
    if (similarPairs.length) {
      contradictions.push('Цели: похожие записи (возможно дубли) — ' + similarPairs.join('; '));
    }
  }
