...
function renderHealth(){
  renderPhysCard();
  const hl=getHL();
  const macros=[{key:'kcal',label:(typeof t==='function'?t('health_kcal'):'Калории'),unit:(typeof t==='function'&&appLang==='en'?'kcal':'ккал'),color:'#e2e2e2',cls:'mc-kcal',icon:'⚡',accent:'rgba(226,226,226,0.7)',t:health.kcal},{key:'prot',label:(typeof t==='function'?t('health_prot'):'Белок'),unit:(typeof t==='function'&&appLang==='en'?'g':'г'),color:'#4ade80',cls:'mc-prot',icon:'💪',accent:'rgba(74,222,128,0.8)',t:health.prot},{key:'fat',label:(typeof t==='function'?t('health_fat'):'Жиры'),unit:(typeof t==='function'&&appLang==='en'?'g':'г'),color:'#fbbf24',cls:'mc-fat',icon:'🔥',accent:'rgba(251,191,36,0.8)',t:health.fat},{key:'carb',label:(typeof t==='function'?t('health_carb'):'Углеводы'),unit:(typeof t==='function'&&appLang==='en'?'g':'г'),color:'#60a5fa',cls:'mc-carb',icon:'⚡',accent:'rgba(96,165,250,0.8)',t:health.carb}];
  ...
  // ── МИКРОНУТРИЕНТЫ ──────────────────────────────────
  const micros=[
    {key:'fiber',label:(typeof t==='function'?t('health_fiber'):'Клетчатка'),unit:(typeof t==='function'&&appLang==='en'?'g':'г'),color:'#34d399',cls:'mc-fiber',icon:'🌿',accent:'rgba(52,211,153,0.8)',t:health.fiber||30},
    {key:'sugar',label:'Сахар',unit:'г',color:'#f472b6',cls:'mc-sugar',icon:'🍬',accent:'rgba(244,114,182,0.8)',t:health.sugar||50},
    {key:'salt',label:'Соль',unit:'г',color:'#fb923c',cls:'mc-salt',icon:'🧂',accent:'rgba(251,146,60,0.8)',t:health.salt||5}
  ];
  ...
  const wl2El=document.getElementById('waterLbl2');if(wl2El)wl2El.textContent=`из ${health.water} стаканов`;
  initWaterBubbles();
...
function setWaterLive(n){
  ...
  const wl2=document.getElementById('waterLbl2');if(wl2)wl2.textContent=`из ${health.water} стаканов`;
...
function showKBZHURecalc(weight){
  ...
  vals.innerHTML=
    `<div style="font-family:var(--mono);font-size:10px;color:var(--t2);margin-bottom:6px">${typeof t==='function'?t('health_bmr'):'BMR:'} <b style="color:var(--t)">${k.bmr} ${typeof t==='function'&&appLang==='en'?'kcal':'ккал'}</b> &nbsp;·&nbsp; ${typeof t==='function'?t('health_tdee'):'TDEE:'} <b style="color:var(--t)">${k.tdee} ${typeof t==='function'&&appLang==='en'?'kcal':'ккал'}</b>${mode==='cut'?' &nbsp;→ &nbsp;'+(typeof t==='function'?t('health_def2'):'дефицит')+' <b style="color:#f472b6">−'+k.deficit+'</b>':mode==='bulk'?' &nbsp;→ &nbsp;'+(typeof t==='function'?t('health_sur2'):'профицит')+' <b style="color:#fb923c">+'+k.surplus+'</b>':''}</div>`+
    `<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:6px">`+
    `<span>${typeof t==='function'?t('health_kcal'):'Калории'}: <b style="color:#e2e2e2">${k.kcal}</b> ${typeof t==='function'&&appLang==='en'?'kcal':'ккал'}</span>`+
    `<span>${typeof t==='function'?t('health_prot'):'Белок'}: <b style="color:var(--blue)">${k.prot}</b> ${typeof t==='function'&&appLang==='en'?'g':'г'}</span>`+
    `<span>${typeof t==='function'?t('health_fat'):'Жиры'}: <b style="color:var(--amber)">${k.fat}</b> ${typeof t==='function'&&appLang==='en'?'g':'г'}</span>`+
    `<span>${typeof t==='function'?t('health_carb'):'Углеводы'}: <b style="color:var(--purple)">${k.carb}</b> ${typeof t==='function'&&appLang==='en'?'g':'г'}</span>`+
    ...
    `<span>${typeof t==='function'?t('health_fiber'):'Клетчатка'}: <b style="color:#34d399">${mc.fiber}</b> ${typeof t==='function'&&appLang==='en'?'g':'г'}</span>`+
    `<span>Сахар ≤<b style="color:#f472b6">${mc.sugar}</b> г</span>`+
    `<span>Соль ≤<b style="color:#fb923c">${mc.salt}</b> г</span>`+
    `</div>`;
...
function updateCuttingUI(){
  ...
  if(lbl){
    if(mode==='cut') lbl.innerHTML=`<span style="color:#f472b6">✂ ${typeof t==='function'&&appLang==='en'?'Mode: Cut':'Режим: Сушка'}</span>`;
    else if(mode==='bulk') lbl.innerHTML=`<span style="color:#fb923c">⬆ ${typeof t==='function'&&appLang==='en'?'Mode: Bulk':'Режим: Набор'}</span>`;
    else lbl.innerHTML=`<span style="color:#34d399">◎ ${typeof t==='function'&&appLang==='en'?'Mode: Maintain':'Режим: Поддержание'}</span>`;
  }
  const desc=document.getElementById('cuttingDesc');
  if(desc){
    if(mode==='cut') desc.textContent=(typeof t==='function'&&appLang==='en'?'Deficit −500 kcal/day → ~0.5 kg fat/week':'Дефицит −500 ккал/день → ~0.5 кг жира в неделю');
    else if(mode==='bulk') desc.textContent=(typeof t==='function'&&appLang==='en'?'Surplus +300 kcal/day → ~0.3 kg muscle/week':'Профицит +300 ккал/день → ~0.3 кг мышц в неделю');
    else desc.textContent=(typeof t==='function'&&appLang==='en'?'Eat at TDEE level, no deficit':'Ешь на уровне TDEE, без дефицита');
  }
...
function applyRecalcKBZHU(){
  ...
  flash('✓ Нормы КБЖУ, вода и микро обновлены');
  renderHealth();
}
