...
// Синхронизирует weekDays всех навыков по cycleSlot и текущему циклу
...
function cycleResetToday(){
  ...
  renderCycleBlock();flash('Цикл сброшен — сегодня слот A');
}
...
function renderCycleCal(){
  const today=nowDK();const{y,m}=_cycleCalMonth;
...
function renderCycleBlock(){
  ...
  if(todayCard){
    if(todayW){
      const userTpls=skillTemplates.filter(t=>t.cycleSlot===todayW.key);
...
function _removeCycleLetterSlot(letter,e){
  ...
  Object.keys(cycleOverrides).forEach(dk=>{if(cycleOverrides[dk]===letter)delete cycleOverrides[dk];});
  LS.s('cycleOverrides',cycleOverrides);
...
function _removeCycleDigitSlot(digit,e){
  ...
  Object.keys(cycleOverrides).forEach(dk=>{if(cycleOverrides[dk]===digit)delete cycleOverrides[dk];});
  LS.s('cycleOverrides',cycleOverrides);
  const cur=document.getElementById('skillsModalCycleSlot').value;
...
function skillsSelectType(type){
  ...
  // Обновить label и placeholder поля упражнений
...
function openSkillsModal(editId){const skb=document.getElementById('skillsDelBtn');if(skb)skb.style.display=editId?'':'none';
  ...
  _tempCycleLetters=[];_tempCycleDigits=[];
  ...
  if(editId){
    ...
    if(t){
      ...
      document.getElementById('skillsModalEx').value=(t.exercises||[]).length && /^\d+[.)]/.test((t.exercises||[])[0])
        ? (t.exercises||[]).join('\n')
        : (t.exercises||[]).join(', ');
      ...
      else{skillsModalSelectSlot(t.cycleSlot||'');}
    }
  }
...
function confirmSkillsModal(){
  ...
  let exercises;
  if(/\d+[.)]\s/.test(exRaw)){
    // Сохраняем с номерами: ["1) жим", "2) тяга"]
    exercises=exRaw.split(/\n/).map(s=>s.trim()).filter(Boolean);
  } else {
  ...
  if(editId){const t=skillTemplates.find(x=>x.id===+editId);if(t){t.name=name;t.exercises=exercises;t.media=media;t.cycleSlot=cycleSlot;t.skillType=skillType;}}
  else{skillTemplates.push({id:nid++,name,exercises,media,cycleSlot,skillType,created:nowDK(),days:[]});LS.s('nid',nid);}
...
function sdmConfirm(id){const tmpl=skillTemplates.find(x=>x.id===id);if(!tmpl)return;const sel=window._sdmSelected||[];if(!sel.length){flash('Выбери хотя бы один день');return;}const mon=getWeekMonday(weekOffset);sel.forEach(idx=>{const d=new Date(mon);d.setDate(mon.getDate()+idx);const key=dk(d);if(!tasks[key])tasks[key]=[];tmpl.exercises.forEach(ex=>{const already=tasks[key].find(t=>t._sportTplId===tmpl.id&&t.name===ex);if(!already)tasks[key].push({id:nid++,name:ex,done:false,emoji:'💪',_sportTplId:tmpl.id});});});LS.s('tasks',tasks);LS.s('nid',nid);const names=sel.map(i=>SKILLS_WDAYS[i]).join(', ');flash(`«${tmpl.name}» → ${names}`);document.getElementById('skillsDayModal').remove();if(curPage==='week')buildWeek();}
  ...
  const renderTpl=t=>{
    ...
    return `<div id="skills-tpl-${t.id}" style="display:flex;align-items:center;gap:10px;padding:9px 0;border-top:0.5px solid rgba(255,255,255,0.05);cursor:default;transition:background .1s${activeBorder}" onmouseenter="this.style.background='rgba(45,212,191,0.03)'" onmouseleave="this.style.background=''">
      ...
      <div style="display:flex;align-items:center;gap:5px;flex-shrink:0">
        ...
        <button onclick="openSkillDashboard(${t.id})" style="width:24px;height:24px;border-radius:5px;background:rgba(255,255,255,0.03);border:0.5px solid rgba(255,255,255,0.07);color:#555;font-size:11px;cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;transition:all .12s" onmouseenter="this.style.background='rgba(45,212,191,0.1)';this.style.borderColor='rgba(45,212,191,0.3)';this.style.color='#2dd4bf'" onmouseleave="this.style.background='rgba(255,255,255,0.03)';this.style.borderColor='rgba(255,255,255,0.07)';this.style.color='#555'" title="Дашборд">◧</button>
