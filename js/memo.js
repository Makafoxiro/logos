...
function _checkPinChange(){
  ...
  if(_pinChangeStep==='old'){
    ...
    (async()=>{
      const newKey=await _deriveCryptoKey(_pinChangeBuf);
      for(let i=0;i<passwords.length;i++){const pd=memoPwDecrypted.find(x=>x.id===passwords[i].id);if(pd){passwords[i].pass=await _encryptPass(newKey,pd.pass);}}
      if(passwords.length)LS.s('passwords',passwords);
      memoCryptoKey=newKey;
...
async function _getCryptoSalt(){
  let s=LS.g('cryptoSalt',null);
  if(!s){s=Array.from(crypto.getRandomValues(new Uint8Array(16))).map(b=>b.toString(16).padStart(2,'0')).join('');LS.s('cryptoSalt',s);}
  return Uint8Array.from(s.match(/.{2}/g),h=>parseInt(h,16));
}
async function _deriveCryptoKey(pin){
  const km=await crypto.subtle.importKey('raw',new TextEncoder().encode(pin),'PBKDF2',false,['deriveKey']);
  return crypto.subtle.deriveKey({name:'PBKDF2',salt:await _getCryptoSalt(),iterations:100000,hash:'SHA-256'},km,{name:'AES-GCM',length:256},false,['encrypt','decrypt']);
}
async function _encryptPass(key,text){
...
async function _decryptPass(key,val){
  if(!val||!val.startsWith('enc:'))return val;
  try{
    const buf=Uint8Array.from(atob(val.slice(4)),c=>c.charCodeAt(0));
...
function buildInlinePin(){
  ...
  [1,2,3,4,5,6,7,8,9,'','0','⌫'].forEach(k=>{
    ...
    if(k!=='')btn.onclick=()=>{
      ...
      if(pv.length===6)setTimeout(async()=>{
        const stored=LS.g('pin','');
        if(stored&&pv===stored||!stored){
          if(!stored){LS.s('pin',pv);flash('PIN установлен');}
          memoUnlocked=true;
          memoCryptoKey=await _deriveCryptoKey(pv);
          memoPwDecrypted=await Promise.all(passwords.map(async p=>({...p,pass:await _decryptPass(memoCryptoKey,p.pass)})));
          // Миграция: шифруем пароли, которые ещё хранятся в открытом виде
          let migrated=false;
          for(let i=0;i<passwords.length;i++){if(passwords[i].pass&&!passwords[i].pass.startsWith('enc:')){passwords[i].pass=await _encryptPass(memoCryptoKey,passwords[i].pass);migrated=true;}}
          if(migrated)LS.s('passwords',passwords);
          renderMemoBody();
        } else {
          pv='';for(let i=0;i<6;i++){const d=document.getElementById('ipd'+i);if(d)d.style.background='transparent';}
...
function switchMemoTab(tab){if(tab!=='passwords'){memoUnlocked=false;memoCryptoKey=null;memoPwDecrypted=[];}curMemoTab=tab;renderMemoTabs();renderMemoBody();}
function renderMemoBody(){
  ...
  if(curMemoTab==='passwords'){
    ...
    const pwCard=p=>`<div class="pw-item"><div class="pw-info"><div class="pw-name">${p.name}</div><div class="pw-login">${p.login}</div><div class="pw-pass">${p.pass}</div></div><div style="display:flex;flex-direction:column;gap:4px;flex-shrink:0"><button class="bd-edit" onclick="openPwEditModal(${p.id})" title="Редактировать"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button></div></div>`;
    const emails=memoPwDecrypted.filter(p=>p.login&&p.login.includes('@'));
    const others=memoPwDecrypted.filter(p=>!(p.login&&p.login.includes('@')));
    let emailHtml='';
    if(emails.length>0){
      const expanded=emailsExpanded||emails.length<=5;
      const visible=expanded?emails:emails.slice(0,5);
      const toggleBtn=emails.length>5?`<span class="sh-btn" onclick="toggleEmailsExpanded()">${expanded?'свернуть':'ещё '+(emails.length-5)+'...'}</span>`:'';
      emailHtml=`<div class="sh" style="padding-top:0">почты ${toggleBtn}</div>`+visible.map(pwCard).join('');
    }
    const othersHeader=`<div class="sh" style="padding-top:${emails.length>0?'12px':'0'}">аккаунты <span class="sh-btn" onclick="openPwModal()">+ добавить</span></div>`;
    div.innerHTML=emailHtml+othersHeader+others.map(pwCard).join('');
    body.appendChild(div);
...
async function confirmAddPw(){const n=document.getElementById('pwName').value.trim();if(!n)return;const login=document.getElementById('pwLogin').value.trim();const passPlain=document.getElementById('pwPass').value.trim();const passEnc=memoCryptoKey?await _encryptPass(memoCryptoKey,passPlain):passPlain;const entry={id:nid++,name:n,login,pass:passEnc};passwords.push(entry);memoPwDecrypted.push({...entry,pass:passPlain});LS.s('passwords',passwords);LS.s('nid',nid);closePwModal();renderMemoBody();}
function delPw(id){deleteItem(passwords,'passwords',id,()=>{memoPwDecrypted=memoPwDecrypted.filter(p=>p.id!==id);renderMemoBody();});}
function toggleEmailsExpanded(){emailsExpanded=!emailsExpanded;renderMemoBody();}
let pwEditId=null;
function openPwEditModal(id){const p=memoPwDecrypted.find(x=>x.id===id)||passwords.find(x=>x.id===id);if(!p)return;pwEditId=id;document.getElementById('pwEditName').value=p.name;document.getElementById('pwEditLogin').value=p.login;document.getElementById('pwEditPass').value=p.pass;openModal('pwEditModal');}
function closePwEditModal(){closeModal('pwEditModal');pwEditId=null;}
async function confirmEditPw(){if(!pwEditId)return;const n=document.getElementById('pwEditName').value.trim();if(!n)return;const login=document.getElementById('pwEditLogin').value.trim();const passPlain=document.getElementById('pwEditPass').value.trim();const passEnc=memoCryptoKey?await _encryptPass(memoCryptoKey,passPlain):passPlain;const p=passwords.find(x=>x.id===pwEditId);const pd=memoPwDecrypted.find(x=>x.id===pwEditId);if(!p)return;p.name=n;p.login=login;p.pass=passEnc;if(pd){pd.name=n;pd.login=login;pd.pass=passPlain;}LS.s('passwords',passwords);closePwEditModal();renderMemoBody();}
// BD
