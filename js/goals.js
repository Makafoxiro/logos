...
function saveGoalItemEdit(){
  ...
  renderGoals();
  flash(typeof t==='function'?t('goals_updated'):'Цель обновлена');
}
function renderGoals(){
  ...
  function goalHtml(g, num){
    ...
    if(g.deadline){
      ...
      if(days!==null){
        ...
        else if(days===0){dlColor='var(--red)';dlLabel=(typeof t==='function'&&appLang==='en'?'→ today!':'→ сегодня!');}
        else if(days<=30){dlColor='var(--amber)';dlLabel=`${formatDate(g.deadline)} → ${days}д.`;}
        else{
          const yrs=Math.floor(days/365);
          const mths=Math.floor((days%365)/30);
          dlColor='var(--a)';
          dlLabel=`${formatDate(g.deadline)} → ${yrs>0?yrs+(typeof t==='function'?t('goals_year_suffix'):(typeof appLang!=='undefined'&&appLang==='en'?'y. ':(typeof t==='function'?t('goals_year_suffix'):'г. '))):''}${mths>0&&yrs<5?mths+(typeof t==='function'?t('goals_month_suffix'):(typeof appLang!=='undefined'&&appLang==='en'?'m.':(typeof t==='function'?t('goals_month_suffix'):'м.'))):''}`.trim();
        }
      }
    ...
    return `<div class="rt-item${g.done?' done':''}" id="goal-item-${g.id}" style="align-items:flex-start;padding:10px 0;cursor:pointer;transition:opacity .2s${g.done?';opacity:0.45':''}" onclick="toggleGoalDone(${g.id})">
      ...
      <div style="flex:1;min-width:0">
        <div style="display:flex;align-items:baseline;gap:6px">
          ${num!==undefined?`<span style="font-family:var(--mono);font-size:9px;color:var(--t3);min-width:14px;flex-shrink:0">${num}.</span>`:''}
  ...
  let html='';
  let _gIdx=0;
  const goalHtmlN = g => { _gIdx++; return goalHtml(g, _gIdx); };
  if(noDl.length){html+=noDl.map(goalHtmlN).join('');}
  if(noDl.length&&withDl.length){html+=`<div style="border-top:1px solid var(--bd);margin:8px 0 4px;opacity:0.4"></div>`;}
  if(withDl.length){html+=withDl.map(goalHtmlN).join('');}
  gl.innerHTML=html||'';
}
...
function addFreeGoal(){
  ...
  clearFreeGoalPhoto();
  renderFreeGoals();
  flash(typeof t==='function'?t('goals_added'):'Цель добавлена');
}
...
function renderFreeGoals(){
  ...
  list.innerHTML=freeGoals.map((g,_fi)=>`
    <div class="rt-item${g.done?' done':''}" id="free-goal-item-${g.id}" style="align-items:flex-start;padding:10px 0;cursor:pointer;transition:opacity .2s${g.done?';opacity:0.45':''}" onclick="toggleFreeGoalDone(${g.id})">
      ...
      <div style="flex:1;min-width:0">
        <div style="display:flex;align-items:baseline;gap:6px">
          <span style="font-family:var(--mono);font-size:9px;color:var(--t3);min-width:14px;flex-shrink:0">${_fi+1}.</span>
...
function confirmRtAnnualAdd(){
  ...
  renderRoutine();
  renderGoals();
  flash(typeof t==='function'&&appLang==='en'?'Reminder added':'Напоминание добавлено');
}
