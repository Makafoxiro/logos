...
const CYCLE_MONTHS_RU = (typeof t==='function'?t('months_full'):['Январь','Февраль','Март','Апрель','Май','Июнь','Июль','Август','Сентябрь','Октябрь','Ноябрь','Декабрь']);
...
function _intensityBadge(dateStr,slotType){
  ...
  return `<div title="${isHeavy?(typeof t==='function'?t('skills_heavy_wk'):'тяжёлая неделя'):(typeof t==='function'?t('skills_light_wk'):'лёгкая неделя')}" style="font-family:var(--mono);font-size:9px;font-weight:700;padding:1px 5px;border-radius:4px;flex-shrink:0;background:${isHeavy?'rgba(232,108,58,0.12)':'rgba(45,212,191,0.08)'};border:0.5px solid ${isHeavy?'rgba(232,108,58,0.35)':'rgba(45,212,191,0.25)'};color:${isHeavy?'#E86C3A':'#2dd4bf'}">${isHeavy?(typeof t==='function'?t('skills_heavy_badge'):'▲ тяж'):(typeof t==='function'?t('skills_light_badge'):'▽ лёгк')}</div>`;
}
...
function cycleResetToday(){
  ...
  renderCycleBlock();renderSkills();flash((typeof t==='function'&&appLang==='en'?'Cycle reset — today is slot A':'Цикл сброшен — сегодня слот A'));
}
...
function renderCycleBlock(){
  ...
  if(todayCard){
    if(todayW){
      ...
      const typeLabel=todayW.type==='mental'?(typeof t==='function'?t('skills_mental_type'):'УМСТВЕННАЯ'):(typeof t==='function'?t('skills_workout'):'ТРЕНИРОВКА');
      ...
      todayCard.innerHTML=`<div style="display:flex;align-items:center;gap:10px"><div style="width:36px;height:36px;border-radius:9px;background:var(--c2);border:1px solid var(--bd);display:flex;align-items:center;justify-content:center;flex-shrink:0"><span style="font-size:18px">😌</span></div><div style="flex:1"><div style="font-family:var(--mono);font-size:8px;color:var(--t3);letter-spacing:1px;margin-bottom:2px">${(typeof t==='function'?t('skills_today_lbl'):'СЕГОДНЯ')}</div><div style="font-size:14px;font-weight:500">${(typeof t==='function'?t('skills_rest_day'):'День отдыха')}</div></div><button onclick="cycleOpenDayMenu('${today}',event)" title="Изменить" style="background:var(--c2);border:1px solid var(--bd);border-radius:7px;color:var(--t3);font-size:13px;padding:4px 8px;cursor:pointer;flex-shrink:0"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button></div>`;
    }
  }
  ...
  if(listEl){
    ...
    for(let i=0;i<14;i++){
      ...
      const ds=dk(d);
      const dow=((typeof t==='function'?t('days_short2'):['Пн','Вт','Ср','Чт','Пт','Сб','Вс']))[(d.getDay()+6)%7];
      const dayNum=d.getDate()+' '+(typeof t==='function'?t('months_short'):['янв','фев','мар','апр','май','июн','июл','авг','сен','окт','ноя','дек'])[d.getMonth()];
      ...
      if(w){
        ...
        rightHtml='<div style="font-size:11px;color:var(--t3);flex:1">'+(typeof t==='function'?t('skills_rest_short'):'отдых')+'</div>';
      }
...
// ══ SKILLS ══════════════════════════════════════════════
const _SKILLS_WDAYS_DEFAULT=['Пн','Вт','Ср','Чт','Пт','Сб','Вс'];
const SKILLS_WDAYS_EN=['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];
const getSkillsWdays=()=>(typeof appLang!=='undefined'&&appLang==='en')?SKILLS_WDAYS_EN:_SKILLS_WDAYS_DEFAULT;
...
function updateSkillsCyclePreviewDigit(){
  ...
  for(let step=0;step<Math.min(pattern.length,7);step++){const m=_slotMeta(pattern[si%pattern.length]);preview.push(getSkillsWdays()[wi%7]+'='+(m?m.key:'?'));si++;wi++;}
  el.textContent=preview.join('  ');el.style.color='var(--a)';
}

function skillsCyclePickWday(i){
  _cycleSetupWday=i;
  getSkillsWdays().forEach((_,j)=>{const el=document.getElementById('scw'+j);if(!el)return;const on=j===i;el.style.background=on?'var(--a2)':'var(--c2)';el.style.borderColor=on?'var(--ab)':'var(--bd)';el.style.color=on?'var(--a)':'var(--t2)';});
...
function applySkillsCycleSetup(){
  ...
  syncWeekDaysFromCycle();
  flash(`✓ Цикл: ${getSkillsWdays()[_cycleSetupWday]} = ${_cycleSetupSlot}`);
  renderCycleBlock();
  renderSkills();
}
...
function sdmConfirm(id){const tmpl=skillTemplates.find(x=>x.id===id);if(!tmpl)return;const sel=window._sdmSelected||[];if(!sel.length){flash('Выбери хотя бы один день');return;}const mon=getWeekMonday(weekOffset);sel.forEach(idx=>{const d=new Date(mon);d.setDate(mon.getDate()+idx);const key=dk(d);if(!tasks[key])tasks[key]=[];tmpl.exercises.forEach(ex=>{const already=tasks[key].find(t=>t._sportTplId===tmpl.id&&t.name===ex);if(!already)tasks[key].push({id:nid++,name:ex,done:false,emoji:'💪',_sportTplId:tmpl.id});});});LS.s('tasks',tasks);LS.s('nid',nid);const names=sel.map(i=>getSkillsWdays()[i]).join(', ');flash(`«${tmpl.name}» → ${names}`);const _sdmEl=document.getElementById('skillsDayModal');if(_sdmEl)_sdmEl.remove();if(curPage==='week')buildWeek();}
  ...
  if(includeDiseases && physDiseases && physDiseases.length){
    physDiseases.forEach(d=>{
      html += `<div class="phys-disease" onclick="this.classList.toggle('open')">
        ...
        <div class="phys-disease-body">${d.notes||(typeof t==='function'?t('skills_no_recs'):'Рекомендации не добавлены')}</div>
      </div>`;
    });
  }
...
function renderSkills(){
  ...
  const renderTpl=t=>{
    ...
    return `<div id="skills-tpl-${t.id}" style="display:flex;align-items:center;gap:10px;padding:9px 0;border-top:0.5px solid rgba(255,255,255,0.05);cursor:default;transition:background .1s${activeBorder}" onmouseenter="this.style.background='rgba(45,212,191,0.03)'" onmouseleave="this.style.background=''">
      ...
      <div style="display:flex;align-items:center;gap:5px;flex-shrink:0">
        <button onclick="toggleSkillsCollapse(${t.id})" style="width:30px;height:30px;border-radius:7px;background:var(--c2);border:1px solid var(--bd);cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;transition:all .15s;flex-shrink:0;color:var(--t2);font-size:12px" onmouseenter="this.style.background='var(--a2)';this.style.borderColor='var(--ab)';this.style.color='var(--a)'" onmouseleave="this.style.background='var(--c2)';this.style.borderColor='var(--bd)';this.style.color='var(--t2)'">${collapsed?'▸':'▾'}</button>
        <button onclick="openSkillDashboard(${t.id})" style="width:30px;height:30px;border-radius:7px;background:var(--c2);border:1px solid var(--bd);cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;transition:all .15s;flex-shrink:0;color:var(--t2);font-size:13px" onmouseenter="this.style.background='var(--a2)';this.style.borderColor='var(--ab)';this.style.color='var(--a)'" onmouseleave="this.style.background='var(--c2)';this.style.borderColor='var(--bd)';this.style.color='var(--t2)'" title="Dashboard">◧</button>
        <button onclick="openSkillsModal(${t.id})" style="width:30px;height:30px;border-radius:7px;background:var(--c2);border:1px solid var(--bd);cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;transition:all .15s;flex-shrink:0;color:var(--t2);font-size:13px" onmouseenter="this.style.background='var(--a2)';this.style.borderColor='var(--ab)';this.style.color='var(--a)'" onmouseleave="this.style.background='var(--c2)';this.style.borderColor='var(--bd)';this.style.color='var(--t2)'">⊞</button>
  ...
  const fLabel=isRestDay?(typeof t==='function'&&appLang==='en'?'mental':'умственные'):(typeof t==='function'&&appLang==='en'?'physical':'физические');
  const sLabel=isRestDay?(typeof t==='function'&&appLang==='en'?'physical':'физические'):(typeof t==='function'&&appLang==='en'?'mental':'умственные');
  ...
  else{html+=`<div style="font-size:12px;color:#1e1e1e;font-family:var(--mono);padding:4px 0 2px;text-align:center;letter-spacing:0.08em">${typeof t==='function'&&appLang==='en'?'no templates':'шаблонов нет'}</div>`;}
...
function openSkillDashboard(id){
  ...
  const typeLabel=sType==='mental'?(typeof t==='function'&&appLang==='en'?'mental':'умственный'):(typeof t==='function'&&appLang==='en'?'physical':'физический');
  const WDAY_NAMES=(typeof t==='function')?t('days_short2'):['Пн','Вт','Ср','Чт','Пт','Сб','Вс'];
