...
function scheduleAllNotifs(){
  ...
  const _cfgDef={deadline:{on:true},weekly:{on:true,time:'09:00',dow:1},monthly:{on:true,time:'09:00',day:1},birthday:{on:true,time:'09:00'},newyear:{on:true,time:'09:00'},homework:{on:true},watchlist:{on:true},patches:{on:true}};
  const _cfgStored=LS.g('notifCfg',{});
  const cfg=Object.assign({},_cfgDef,_cfgStored);
  // Добавляем недостающие вложенные ключи из дефолта
  Object.keys(_cfgDef).forEach(k=>{if(!cfg[k]||typeof cfg[k]!=='object')cfg[k]=_cfgDef[k];});

  // ДЕДЛАЙНЫ — проверять каждые 6 часов
  if(cfg.deadline&&cfg.deadline.on){
      ...
      if(checkDays.includes(daysLeft)){
        ...
        if(target>now){
          ...
          const t=setTimeout(()=>{
            ...
            const body='Подведи итоги месяца!';
...
function renderNotifSettings(){
  ...
  const _cfgDef={deadline:{on:true},weekly:{on:true,time:'09:00',dow:1},monthly:{on:true,time:'09:00',day:1},birthday:{on:true,time:'09:00'},newyear:{on:true,time:'09:00'},homework:{on:true},watchlist:{on:true},patches:{on:true}};
  const _cfgRaw=LS.g('notifCfg',{});
  const cfg=Object.assign({},_cfgDef,_cfgRaw);
  // Добавляем недостающие вложенные ключи
  Object.keys(_cfgDef).forEach(k=>{ if(!cfg[k])cfg[k]=_cfgDef[k]; });
  ...
  el.innerHTML=`
    <div class="srow">
      <div><div class="slabel">${t('notif-lbl')}</div><div class="ssub">${perm==='granted'?t('notif-perm-granted'):perm==='denied'?t('notif-perm-denied'):t('notif-perm-ask')}</div></div>
      ${enabled
        ?`<button class="add-btn" style="background:var(--c2);color:var(--red);font-size:11px" onclick="disableNotifs()">${t('notif-off-btn')}</button>`
        :`<button class="add-btn" style="font-size:11px" onclick="requestNotifPermission()">${t('notif-on-btn')}</button>`
      }
    </div>
    ${enabled?`
    <div class="srow"><div><div class="slabel">${t('notif-deadlines')}</div><div class="ssub">${t('notif-deadlines-sub')}</div></div>${tgl('deadline',cfg.deadline.on)}</div>
    <div class="srow"><div><div class="slabel">${t('notif-weekly')}</div><div class="ssub">${t('notif-weekly-sub')} ${timeInput('weekly',cfg.weekly&&cfg.weekly.time)}</div></div>${tgl('weekly',cfg.weekly&&cfg.weekly.on)}</div>
    <div class="srow"><div><div class="slabel">${t('notif-monthly')}</div><div class="ssub">${t('notif-monthly-sub')} ${timeInput('monthly',cfg.monthly&&cfg.monthly.time)}</div></div>${tgl('monthly',cfg.monthly&&cfg.monthly.on)}</div>
    <div class="srow"><div><div class="slabel">${t('notif-birthday')}</div><div class="ssub">${t('notif-birthday-sub')} ${timeInput('birthday',cfg.birthday&&cfg.birthday.time)}</div></div>${tgl('birthday',cfg.birthday&&cfg.birthday.on)}</div>
    <div class="srow"><div><div class="slabel">${t('notif-newyear')}</div><div class="ssub">${t('notif-newyear-sub')} ${timeInput('newyear',cfg.newyear&&cfg.newyear.time)}</div></div>${tgl('newyear',cfg.newyear&&cfg.newyear.on)}</div>
    <div class="srow"><div><div class="slabel">${t('notif-homework')}</div><div class="ssub">${t('notif-homework-sub')}</div></div>${tgl('homework',cfg.homework&&cfg.homework.on)}</div>
    <div class="srow"><div><div class="slabel">${t('notif-watchlist')}</div><div class="ssub">${t('notif-watchlist-sub')}</div></div>${tgl('watchlist',cfg.watchlist&&cfg.watchlist.on)}</div>
    <div class="srow"><div><div class="slabel">${t('notif-patches')}</div><div class="ssub">${t('notif-patches-sub')}</div></div>${tgl('patches',cfg.patches&&cfg.patches.on)}</div>
    `:''}  `;
}
...
function renderRemindersCard(){
  ...
  el.innerHTML=`
    <div class="srow">
      <div><div class="slabel">${t('reminders-lbl')}</div><div class="ssub">${t('reminders-sub')}</div></div>
    </div>
    <div class="srow">
      <div>
        <div class="slabel">${t('reminder-weight')}</div>
        <div class="ssub">${t('reminder-weight-sub')}</div>
      </div>
