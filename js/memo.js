...
function _checkPinChange(){
  ...
  if(_pinChangeStep==='old'){
    ...
    flash(typeof t==='function'&&appLang==='en'?'PIN changed ✓':'PIN изменён ✓');
  ...
  [1,2,3,4,5,6,7,8,9,'','0','⌫'].forEach(k=>{
    ...
    if(k!=='')btn.onclick=()=>{
      ...
      if(pv.length===6)setTimeout(()=>{
        ...
        else if(!stored){LS.s('pin',pv);flash(typeof t==='function'&&appLang==='en'?'PIN set':'PIN установлен');memoUnlocked=true;renderMemoBody();}
...
function renderMemoTabs(){
  const _tl=(k,fb)=>(typeof t==='function'?t(k):fb);
  const tabs=[{k:'passwords',l:_tl('memo_passwords','ПАРОЛИ')},{k:'birthdays',l:_tl('memo_bdays','ДР')},{k:'homework',l:_tl('memo_hw','ДЗ')},{k:'watchlist',l:_tl('memo_watchlist',(typeof t==='function'?t('memo_watchlist'):'ВОТЧЛИСТ'))},{k:'notes',l:_tl('memo_notes','ЗАМЕТКИ')},{k:'patches',l:_tl('memo_patches','ПАТЧИ')}];
